...
router.get('/api/crud/:user', (req, res) => {
  // random pic fetcher
...
router.get('/api/crud/profilePics/:user', verifyAuthentication, (req, res) => {
  // profile pics fetcher
...
router.post('/api/crud', verifyAuthentication, (req, res) => {
  // Creates new pic
...
router.put('/api/crud/:_id', (req, res) => {
  // update pic
  const picID = req.params._id;
  pictures.findById(picID).then((picObject) => {
    const newAve =
      (picObject.avgRating * picObject.totalRatings +
        Number(req.body.newrating)) /
      (picObject.totalRatings + 1);
    const updateInfo = {
...
router.delete('/api/crud/:_id', verifyAuthentication, (req, res) => {
  //deletes pic
...
router.get(
  '/api/crud/orderedpics/:order/:limit',
  verifyAuthentication,
  (req, res) => {
    const sortType =
      req.params.order == 'ascending' ? 'avgRating' : '-avgRating';

    // get ordered list
    pictures
      .find({ totalRatings: { $gt: 5 } })
      .sort(sortType)
...
function randomPic(picArr, user) {
  ...
  while (true) {
    ...
    if (alreadyVoted.length === picArr.length) {
      // all pics in DB have been voted on at this point
